...
std::vector<std::pair<std::string, std::string> > Socket::scanDevices()
{
    ...
    int max_rsp, num_rsp;
    int dev_id, tempSocket, len, flags;
    int i;
    char addr[19] = { 0 };
    char name[248] = { 0 };
    ...
    for (i = 0; i < num_rsp; i++) 
    {
        ...
        if (hci_read_remote_name(tempSocket, &(ii+i)->bdaddr, sizeof(name), name, 0) < 0)
        {
        	strcpy(name, "[unknown]");
        }
        printf("%s  %s\n", addr, name);
        tempPair = std::make_pair<std::string, std::string>(addr, name);
