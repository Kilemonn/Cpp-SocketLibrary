
#include <chrono>

#include <gtest/gtest.h>

#include "../../src/serversocket/TCPServerSocket.h"
#include "../../src/socketexceptions/BindingException.hpp"
#include "../../src/socketexceptions/TimeoutException.hpp"

namespace kt
{
    class TCPServerSocketTest: public ::testing::Test
    {
    protected:
        TCPServerSocket serverSocket;
    protected:
        TCPServerSocketTest() : serverSocket() {}
        // void SetUp() override { }
        void TearDown() override
        {
            serverSocket.close();
        }
    };

    /*
     * Ensure the defaults are set correctly.
     */
    TEST_F(TCPServerSocketTest, TestDefaultConstructor)
    {
        ASSERT_NE(InternetProtocolVersion::Any, serverSocket.getInternetProtocolVersion());
        ASSERT_NE(0, serverSocket.getPort());
        ASSERT_FALSE(kt::isInvalidSocket(serverSocket.getSocket()));
    }

    /*
     * Ensure that we throw a binding exception if the port is already used.
     */
    TEST_F(TCPServerSocketTest, TestConstructors)
    {
        EXPECT_THROW({
            TCPServerSocket server2(std::nullopt, serverSocket.getPort());
        }, BindingException);
    }

    /*
     * Ensure thatt if a copied serversocket is closed, that it closes the copied socket too since they shared the same underlying descriptor.
     */
    TEST_F(TCPServerSocketTest, TestCloseCopiedServerSocket)
    {
        TCPServerSocket copiedServer = serverSocket;
        serverSocket.close();
        ASSERT_THROW(copiedServer.accept(), SocketException);
    }

    /*
     * Ensure the copy constructed server socket is able to connect to the client and receive messages.
     */
    TEST_F(TCPServerSocketTest, TestCopyConstructor)
    {
        TCPServerSocket server2(serverSocket);
        ASSERT_EQ(serverSocket.getInternetProtocolVersion(), server2.getInternetProtocolVersion());
        ASSERT_EQ(serverSocket.getPort(), server2.getPort());
        ASSERT_EQ(serverSocket.getSocket(), server2.getSocket());

        TCPSocket client(kt::getLocalAddress(serverSocket.getInternetProtocolVersion()), serverSocket.getPort());

        TCPSocket serverClient = server2.accept();
        const std::string testString = "test";

        ASSERT_EQ(client.send(testString), testString.size());
        const std::string responseString = serverClient.receiveAmount(testString.size());
        ASSERT_EQ(responseString, testString);

        serverClient.close();
        client.close();
        server2.close();
    }

    /*
     * Ensure the server socket can be created using IPV6 and can accept a connection.
     */
    TEST_F(TCPServerSocketTest, TestServerSocketAsIPV6)
    {
        TCPServerSocket ipv6Server(std::nullopt, 0, 20, InternetProtocolVersion::IPV6);

        TCPSocket client("0000:0000:0000:0000:0000:0000:0000:0001", ipv6Server.getPort());
        TCPSocket serverClient = ipv6Server.accept();

        const std::string testString = "test";
        ASSERT_EQ(client.send(testString), testString.size());
        const std::string responseString = serverClient.receiveAmount(testString.size());
        ASSERT_EQ(responseString, testString);

        serverClient.close();
        client.close();
        ipv6Server.close();
    }

    /*
     * Ensure the server socket cannot be connected to by a client using IPV4.
     */
    TEST_F(TCPServerSocketTest, TestServerSocketAsIPV4ServerAndIPV4Client)
    {
        serverSocket.close();

        TCPServerSocket ipv4Server(std::nullopt, 0, 20, InternetProtocolVersion::IPV4);
        ASSERT_EQ(InternetProtocolVersion::IPV4, ipv4Server.getInternetProtocolVersion());

        // Attempt to connect to a local server using a IPV6 address (which is not being hosted)
        EXPECT_THROW({
            TCPSocket client("::1", ipv4Server.getPort());
        }, SocketException);
        
        // Make sure theres no incoming connections
        EXPECT_THROW({
            ipv4Server.accept(1000);
        }, TimeoutException);

        ipv4Server.close();
    }

    /**
     * Ensure that calls to ServerSocket.accept() with a provided timeout waits for atleast the provided timeout before throwing a TimeoutException.
    */
    TEST_F(TCPServerSocketTest, TestServerSocketAcceptTimeout)
    {
        const int seconds = 1;
        std::chrono::steady_clock::time_point start = std::chrono::steady_clock::now();
        EXPECT_THROW({
            TCPSocket serverClient = serverSocket.accept(seconds * 1000000);
        }, TimeoutException);

        std::chrono::steady_clock::time_point end = std::chrono::steady_clock::now();
        ASSERT_GE(seconds, std::chrono::duration_cast<std::chrono::seconds>(end - start).count());
    }
}
